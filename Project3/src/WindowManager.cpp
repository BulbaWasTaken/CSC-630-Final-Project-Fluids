...
bool WindowManager::init(int const width, int const height)
{
	...
	glfwSetCursorPosCallback(windowHandle, move_callback);
...
void WindowManager::move_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (instance && instance->callbacks)
	{
		instance->callbacks->moveCallback(window, xpos, ypos);
	}
}